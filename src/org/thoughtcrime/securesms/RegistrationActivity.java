...
import org.thoughtcrime.securesms.registerService.JoinActivity;
...
//    private VerificationCodeView verificationCodeView;
    ...
    private void initializeResources() {
        ...
        this.etCode = findViewById(R.id.et_code);
        this.verifyButton = findViewById(R.id.verifyButton);
...
//        this.verificationCodeView = findViewById(R.id.code);
        ...
        this.callMeCountDownView.setOnClickListener(v -> joinRequest());
        this.verifyButton.setOnClickListener(v -> onCodeComplete());
...
//                            verificationCodeView.clear();
    ...
    public void onCodeComplete(/*@NonNull String code*/) {
        if (TextUtils.isEmpty(etCode.getText())) {
            etCode.setError("Enter verify code!");
        }
        ...
        contact.setVerificationCode(etCode.getText().toString());

        ApplicationContext.getInstance().getService().verifyUser(contact).enqueue(new Callback<ContactFromApi>() {
            @Override
...
//                            verificationCodeView.clear();
    ...
    private void joinRequest() {
        /*if (registrationState.state == RegistrationState.State.VERIFYING) {
            callMeCountDownView.startCountDown(0);
    ...
    private void displayVerificationView() {
        ...
        subtitle.animate().translationX(-1 * subtitle.getWidth()).setDuration(SCENE_TRANSITION_DURATION).setListener(new AnimationCompleteListener() {
            @Override
            public void onAnimationEnd(Animator animation) {
                SpannableString subtitleDescription = new SpannableString(getString(R.string.RegistrationActivity_please_enter_the_verification_code_sent_to_s));
    ...
    /*registrationContainer.animate().translationX(-1 * registrationContainer.getWidth()).setDuration(SCENE_TRANSITION_DURATION).setListener(new AnimationCompleteListener() {
      ...
      public void onAnimationEnd(Animator animation) {
        ...
        this.callMeCountDownView.startCountDown(0);
    }
